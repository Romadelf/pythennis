...
def oderhs(t, y):
    ...
    if x3 <= tol:                  # ici on créé les rebonds
   ...
   # indice     0  1  2  3      4       5     6  7  8
    ...
    dy[6]=0             # ici la derivé de omega qui vaut zero car consideré comme constante
    dy[7]=0
    dy[8]=0
...
def trajectoireFiletHorizontal (yInit , T ):
    ...
    variable = solve_ivp(oderhs,[t0,T],Cond ,t_eval=nombre_iteration)
    ...
    for i in range(compteur_max):
        ...
        if x1[i]<=tol:  #si on est derriere le filet 
            ...
            if x1[i]> D : #on est hors du terrain
